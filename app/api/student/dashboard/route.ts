...
export async function GET(): Promise<Response> {
  try {
    ...
    if (!student) {
      return errorResponse("Student profile not found", 404, undefined, "STUDENT_NOT_FOUND");
    }

    const [enrollments, submissionCount, overdueSubmissionCount, latestResult] = await Promise.all([
